    ...
    public partial class Inventory : Form
    {
        ...
        public void stockChangeSelect()    // 전체 재고를 보여준다
        {
            ...
            try
            {
                ...
                inventory_dataGridView.Columns.Add("상품이름", "상품이름");
                inventory_dataGridView.Columns.Add("증가수량", "증가수량");
                inventory_dataGridView.Columns.Add("증가시간", "증가시간");
                inventory_dataGridView.Columns.Add("감소수량", "감소수량");
                inventory_dataGridView.Columns.Add("감소시간", "감소시간");

                MySqlCommand cmd = new MySqlCommand("SELECT * " +
                                                    "FROM inventory_history;", conn);
                ...
                reader["menu_name"].ToString(),
                reader["up_quantity"].ToString(),
                reader["stock_up_time"].ToString(),
                reader["down_quantity"].ToString(),
                reader["stock_down_time"].ToString(),
            };
                    inventory_dataGridView.Rows.Add(row);
                }

                inventory_dataGridView.Columns["상품이름"].Width = 150;
                inventory_dataGridView.Columns["증가수량"].Width = 80;
                inventory_dataGridView.Columns["증가시간"].Width = 150;
                inventory_dataGridView.Columns["감소수량"].Width = 80;
                inventory_dataGridView.Columns["감소시간"].Width = 150;
            }
            catch (Exception ex)
            {
                // columns 초기화
        ...
        private void updateButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("추가할 수량을 입력하세요.", "입력", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                ...
                string insertHistorySql = "INSERT INTO inventory_history (menu_id, menu_name, up_quantity, stock_up_time) " +
                                          "VALUES (@menu_id, @menu_name, @up_quantity, @stock_up_time);";
                ...
                insertHistoryCmd.Parameters.AddWithValue("@up_quantity", quantityToAdd);
                insertHistoryCmd.Parameters.AddWithValue("@stock_up_time", DateTime.Now);
